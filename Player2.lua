...
function Player2:init(map)
    ...
    self.x = map.tileWidth * 10 - self.width
    ...
    self.behaviors = {
        ['idle'] = function(dt)
            ...
            elseif love.keyboard.isDown('k') and self.count >= 50 then
...
function Player2:update(dt)
    ...
    if self.skin == 4 and gameState == 'play' then
        if love.keyboard.isDown('l') and self.snakeCanShoot and self.scaleX == 1 then 
    ...
    if self.skin == 1 and gameState == 'play' then
        if love.keyboard.isDown('l') and self.bulCanShoot and self.scaleX == 1 then 
...
-- books
    if self.skin == 2 and gameState == 'play' then
        if love.keyboard.isDown('l') and self.canShoot and self.scaleX == 1 then 
    ...
    if self.skin == 3 and gameState == 'play' then
        if love.keyboard.isDown('l') and self.dashCount > 50 then 
...
function Player2:render()
    if gameState ~= 'select' and gameState ~= 'title' then
